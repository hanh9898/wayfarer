#!/usr/bin/env python3
"""validate-scripture.py — kiểm bí kíp theo reference/scripture.schema.md (schema 1).
...
Dùng:
    validate-scripture.py                      kiểm cả kho ~/.wayfarer/scriptures/
    validate-scripture.py <đường-dẫn>          kiểm một quyển
    validate-scripture.py <đường-dẫn> --json   đầu ra JSON cho skill
...
try:
    ...
    sys.stderr.write(
        ...
        "Chưa cài được thì kiểm tay theo §6 của reference/scripture.schema.md,\n"
...
def main():
    ...
    ap.add_argument("--kho", default="~/.wayfarer/scriptures", help="thư mục kho")
    a = ap.parse_args()
