...
| `giam_dinh` | chép nguyên từ `metrics` script trả về, cộng ba trường skill tự suy: `muc_chi_phi`, `cau_truc`, `cau_truc_nguon` |
...
| `logged_at` | ISO 8601 kèm offset múi giờ, hoặc `Z` nếu UTC |
...
| Trường | Ghi gì |
|---|---|
| `id` file | kebab-case từ `ten_sach`, **kèm hậu tố thời gian** (`<title>-<YYYYMMDDHHmm>.json`). Không có hậu tố thì gửi lại cùng cuốn lần thứ hai sẽ ghi đè thư cũ, mà hộp thư chỉ được ghi thêm |
...
| `status` | `cho` khi vừa gửi. Vai này không đổi giá trị đó — xử lý thư là việc của bên nhận |

Vai này **không** đọc, không ghi `truong-mon/counsel.jsonl` hay `counsel-broken.jsonl` dưới bất kỳ hình thức nào.
