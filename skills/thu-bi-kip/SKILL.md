...
**Giám định xong một cuốn rút được chữ:** đã trình đủ **bốn** phép giám định kèm ước chi phí, số trang trình ra có kèm đúng nhãn đơn vị, người học đã xác nhận, và `~/.vandao/tang-kinh/nhap-dang-do/<id>.json` có một bản ghi mới đủ trường — kiểm bằng cách đọc lại chính file vừa ghi và đối chiếu với thứ vừa trình.

**Từ chối một bản không rút được chữ:** đã khuyên tìm bản khác, đã ghi một thư vào `~/.vandao/ban-giao/thu/` và đọc lại nó, **và** `nhap-dang-do/` không có bản ghi mới nào.
...
- Muốn hệ tự tìm hoặc tải sách — không có luồng đó; muốn được gợi ý tìm sách nào thì dẫn sang `/van-dao:truong-mon`.
...
2. Lớp cá nhân: `~/.vandao/custom/thu-bi-kip.toml` (chỉ đọc nếu tồn tại), ghi thưa được.
...
**Xác định thư mục home hiện tại của máy đang chạy** (`$env:USERPROFILE` trên Windows, `$HOME` trên POSIX) trước khi đọc/ghi đường dẫn `~/.vandao/` nào — đừng giả định đường dẫn đã biết từ phiên trước. Dùng đúng một giá trị đó cho cả lượt. Đường dẫn sách người học đưa: đổi về **tuyệt đối** trước đã.

### Bước 1 — Kiểm kho trước, vì nó rẻ nhất

Đọc `~/.vandao/tang-kinh/nhap-dang-do/` (chưa có thì coi là rỗng), so với đường dẫn vừa chuẩn hoá:
...
```
python <gốc-plugin>/bin/giam-dinh.py "<đường-dẫn-tuyệt-đối>"
```
...
Nạp `references/dinh-dang.md`, ghi `~/.vandao/tang-kinh/nhap-dang-do/<id>.json` đủ trường (tạo thư mục nếu chưa có).
...
3. **Gửi thư báo Trưởng môn**, theo khuôn trong `references/dinh-dang.md`.
